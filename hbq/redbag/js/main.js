var Config = {
	imgPath: 'http://7xnz8p.com1.z0.glb.clouddn.com/hbq%2Fredbag%2Fimage%2F',
	audioPath: 'http://7xnz8p.com1.z0.glb.clouddn.com/hbq%2Fredbag%2Faudio%2F',
	preload: [
		['audio', 'run.mp3'],
		['audio', 'slide.mp3'],
		['audio', 'bomb.mp3'],
...
document.documentElement.ontouchstart = function(e){
	if($('.pop-show').size()>0||$('#result').hasClass('result-open')){
		return;
	}
   e.preventDefault();
};
...
var page = (function(){
	var $domScene,
		...
		initPrevScene = function(){
			init(function(){
				$domScene = $('.scene');
				s = parseInt(GetQueryString('s')) || 0;
				switchScene(s);
				if(s==1){
					$domScene.eq(curScene).addClass('again-scene');
				}
				playBg('begin.mp3');
			});
		},
		...
		choseMan = function(dir) {
			...
			isChoseManAni = true;
			setTimeout(function(){isChoseManAni = false;},300);
		},
		// game
		...
		initGame = function(){
			...
			$('#result').on('touchend', '.result-btn', function() {
				...
				window.location.href = 'index.shtml?a=' + isAudio + '&s=1';
			...
			for (var i = 0; i < count; i++) {
				...
				for (var k in game.redBag) {
					if (randomRatio <= (game.redBag[k].ratio[parseInt(manKind)] + ratio) * game.baseRatio) {
						if(k==bombKey){
							if(bombNum > game.redBag[bombKey].max[manKind]){
								i--;
								continue;
							}else{
								bombNum ++;
		...
		popHide = function(pop) {
			...
			if($('.pop-show').size()<=0){
				$('#result').removeClass('result-no-ani');
			}
		},
